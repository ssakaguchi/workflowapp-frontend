...
現在は、認証、申請一覧、申請詳細、申請作成、申請編集、申請削除、承認者選択、承認ルート表示、ステータス変更、ページネーション、ステータス絞り込みを実装しています。

---

## 画面イメージ

画像を追加する場合は、以下のパスに画像ファイルを配置してください。  
画像未作成の段階でも README の構成が崩れないよう、各画面ごとに画像貼付用の枠を用意しています。
...
![承認・却下確認ダイアログ](docs/images/application-status-confirm-dialog.png)

### 削除確認ダイアログ

![削除確認ダイアログ](docs/images/application-delete-confirm-dialog.png)
...
- ページネーション、絞り込み、承認・却下などの業務アプリケーション向け機能の実装
...
| メソッド | エンドポイント         | 内容           |
| -------- | ---------------------- | -------------- |
| `GET`    | `/api/users/approvers` | 承認者一覧取得 |
...
| `status`   | ステータス絞り込み。`All` / `Pending` / `Approved` / `Rejected` などを指定 |
...
src/
  ...
  components/
    applications/
      ...
      ApplicationDeleteConfirmDialog.tsx
      ApplicationDetailInfo.tsx
      ApplicationListTable.tsx
      ApplicationStatusFilter.tsx
      ApprovalRouteTable.tsx
    auth/
      ProtectedRoute.tsx
      PublicRoute.tsx
    layout/
...
- `ApplicationDetailPage`
  ...
  - 一覧画面への遷移
  - 編集画面への遷移
  - ロール未保存時にログイン中ユーザー情報からロールを復元すること
  - 承認者の場合のみ承認 / 却下ボタンを表示すること
  - ステータス更新確認ダイアログの表示
  - ステータス更新成功時の表示更新
  - ステータス更新失敗時のエラー表示
...
GitHub Actions で、Pull Request および `main` ブランチへの push 時に以下を実行します。
